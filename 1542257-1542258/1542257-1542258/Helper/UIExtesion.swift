...
extension UIViewController {
    ...
    func alert(title: String, message: String, handler: @escaping (UIAlertAction) -> Void ) {
        let okAction = UIAlertAction(title: NSLocalizedString("ok", comment: ""), style: .default, handler: handler)
        let alert = UIAlertController(title: title, message: message, preferredStyle: .alert)
        alert.addAction(okAction)
        self.present(alert, animated: true, completion: nil)
    }
    ...
    func addDoneButton(to control: UITextField){
        ...
        toolbar.items = [
            ...
            UIBarButtonItem(title: NSLocalizedString("done", comment: ""), style: .done, target: control,
    ...
    func addDoneButton(_ textview: UITextView){
        ...
        toolbar.items = [
            UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: self, action: nil),
            UIBarButtonItem(title: NSLocalizedString("done", comment: ""), style: .done, target: textview,
                            action: #selector(UITextField.resignFirstResponder))
        ]
